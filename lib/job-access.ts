...
export function accessTokenFromRequest(request: Request, jobId: string) {
  ...
  return cookieHeader.split(";").map((part) => part.trim()).find((part) => part.startsWith(accessCookiePrefix))?.split("=").slice(1).join("=") || undefined;
}

export function accessCookie(jobId: string, token: string) {
  return `${accessCookiePrefix}${jobId}=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=1800`;
}
