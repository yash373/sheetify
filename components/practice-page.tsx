...
export function PracticePage({ sheetId }: { sheetId: string }) {
  ...
  return <main className="min-h-screen bg-background"><header className="border-b border-border/80 bg-card/50"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8"><Link href="/" className="flex items-center gap-1 text-sm font-medium text-muted-foreground hover:text-foreground"><ChevronLeft className="size-4" />Sheetify</Link><div className="flex items-center gap-3"><Badge variant="outline">{difficultyLabels[sheet.difficulty]}</Badge><Button variant="ghost" size="icon" aria-label="Practice help"><CircleHelp className="size-4" /></Button></div></div></header><section className="mx-auto max-w-7xl px-5 py-8 sm:px-8"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Practice sheet</p><h1 className="mt-2 font-heading text-4xl tracking-[-0.03em] sm:text-5xl">{sheet.song.title}</h1><p className="mt-2 text-sm text-muted-foreground">{sheet.song.artist} · {sheet.key} · {sheet.timeSignature}</p></div><div className="flex items-center gap-4 text-sm text-muted-foreground"><span>{tempo} BPM</span><span>Measure {currentMeasure}</span><Volume2 className="size-4" /></div></div><div className="mt-8 overflow-hidden rounded-2xl border border-border bg-card shadow-[0_20px_70px_oklch(0.3_0.04_265/0.08)]"><ScoreRenderer musicXml={sheet.musicXml} currentMeasure={currentMeasure} currentNoteIndex={currentNoteIndex} totalNotes={sheet.noteEvents.length} /></div><div className="mt-6 rounded-2xl border border-border bg-card p-5"><div className="flex flex-wrap items-center gap-3"><Button variant="secondary" size="icon" aria-label="Restart" onClick={() => { setElapsed(0); lastNote.current = -1; }}><RotateCcw className="size-4" /></Button><Button size="icon-lg" aria-label={playing ? "Pause" : "Play"} onClick={() => setPlaying((current) => !current)}>{playing ? <Pause className="size-5" /> : <Play className="size-5" />}</Button><Button variant="outline" size="icon" aria-label="Stop" onClick={() => { setPlaying(false); setElapsed(0); lastNote.current = -1; }}><Square className="size-4" /></Button><div className="min-w-44 flex-1"><Progress value={(elapsed / duration) * 100} aria-label="Playback progress" className="h-2" /><input aria-label="Playback position" type="range" min="0" max={duration} step="0.1" value={elapsed} onChange={(event) => { setElapsed(Number(event.target.value)); lastNote.current = -1; }} className="mt-3 w-full accent-[var(--primary)]" /></div><span className="tabular-nums text-xs text-muted-foreground">{elapsed.toFixed(1)} / {duration.toFixed(1)}</span></div><div className="mt-5 grid gap-5 border-t border-border/70 pt-5 sm:grid-cols-[1fr_auto] sm:items-end"><label className="text-xs font-medium text-muted-foreground">Tempo <span className="ml-2 text-foreground">{tempo} BPM</span><Slider aria-label="Tempo" min={50} max={150} value={[tempo]} onValueChange={(value) => setTempo(Array.isArray(value) ? value[0] : tempo)} className="mt-3" /></label><div className="flex flex-wrap gap-2"><Button variant={countIn ? "default" : "outline"} aria-pressed={countIn} onClick={() => setCountIn((current) => !current)}><Timer className="size-4" />Count-in</Button><Button variant={metronome ? "default" : "outline"} aria-pressed={metronome} onClick={() => setMetronome((current) => !current)}><Drum className="size-4" />Metronome</Button><Button variant={looping ? "default" : "outline"} onClick={() => setLooping((current) => !current)}><SkipBack className="size-4" />Loop {looping ? "on" : "off"}</Button><Button variant="ghost" onClick={() => { setLoopStart(0); setLoopEnd(loopLength); }}>Reset loop</Button></div></div><div className="mt-5 flex flex-wrap items-center gap-2 border-t border-border/70 pt-4"><span className="mr-1 text-xs font-medium text-muted-foreground">Jump to measure</span>{Array.from({ length: measureCount }, (_, index) => index + 1).map((measure) => <Button key={measure} variant={measure === currentMeasure ? "secondary" : "ghost"} size="sm" aria-label={`Jump to measure ${measure}`} onClick={() => jumpToMeasure(measure)}>{measure}</Button>)}</div><p className="mt-4 text-xs text-muted-foreground" aria-live="polite">{countIn ? "Count-in on" : "Count-in off"} · {metronome ? "Metronome on" : "Metronome off"} · Space play/pause · R restart · Loop repeats {loopStart.toFixed(0)}–{loopEnd.toFixed(0)} seconds</p></div></section></main>;
}
