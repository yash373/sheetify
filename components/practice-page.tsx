...
export function PracticePage({ sheetId }: { sheetId: string }) {
  ...
  return <main className="min-h-screen bg-[#2a2d31] text-white"><header className="border-b border-white/10 bg-[#1f2124]"><div className="mx-auto flex max-w-[1500px] items-center justify-between px-4 py-3 sm:px-6"><Link href="/" className="flex items-center gap-2 text-sm font-semibold text-white/80 hover:text-white"><ChevronLeft className="size-4" />Sheetify <span className="hidden text-xs font-normal text-white/40 sm:inline">/ Practice workspace</span></Link><div className="flex items-center gap-3"><Badge variant="outline" className="border-white/20 text-white/70">{difficultyLabels[sheet.difficulty]}</Badge><Button variant="ghost" size="icon" aria-label="Practice help" className="text-white/70 hover:bg-white/10 hover:text-white"><CircleHelp className="size-4" /></Button></div></div></header><section className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6"><div className="mb-4 flex flex-wrap items-center justify-between gap-4 rounded-md border border-white/10 bg-[#202226] px-4 py-3"><div><p className="text-[10px] font-semibold tracking-[0.18em] text-white/40">SCORE VIEW</p><h1 className="mt-1 text-xl font-semibold tracking-tight text-white sm:text-2xl">{sheet.song.title}</h1><p className="text-xs text-white/50">{sheet.song.artist} · {sheet.key} · {sheet.timeSignature}</p></div><div className="flex items-center gap-5 text-xs text-white/60"><span>{tempo} BPM</span><span>Measure {currentMeasure}</span><span className="hidden sm:inline">Note {currentNoteIndex + 1}/{sheet.noteEvents.length}</span><Volume2 className="size-4" /></div></div><div className="overflow-hidden rounded-md border border-black/30 bg-white text-slate-950 shadow-[0_24px_70px_rgba(0,0,0,0.28)]"><ScoreRenderer musicXml={sheet.musicXml} currentMeasure={currentMeasure} currentNoteIndex={currentNoteIndex} totalNotes={sheet.noteEvents.length} /></div><div className="mt-4 rounded-md border border-white/10 bg-[#202226] p-4"><div className="flex flex-wrap items-center gap-3"><Button variant="secondary" size="icon" aria-label="Restart" onClick={() => { setElapsed(0); lastNote.current = -1; }}><RotateCcw className="size-4" /></Button><Button size="icon-lg" aria-label={playing ? "Pause" : "Play"} onClick={() => setPlaying((current) => !current)}>{playing ? <Pause className="size-5" /> : <Play className="size-5" />}</Button><Button variant="outline" size="icon" aria-label="Stop" onClick={() => { setPlaying(false); setElapsed(0); lastNote.current = -1; }}><Square className="size-4" /></Button><div className="min-w-44 flex-1"><Progress value={(elapsed / duration) * 100} aria-label="Playback progress" className="h-2 bg-white/10" /><input aria-label="Playback position" type="range" min="0" max={duration} step="0.1" value={elapsed} onChange={(event) => { setElapsed(Number(event.target.value)); lastNote.current = -1; }} className="mt-3 w-full accent-[var(--primary)]" /></div><span className="tabular-nums text-xs text-white/50">{elapsed.toFixed(1)} / {duration.toFixed(1)}</span></div><div className="mt-4 grid gap-4 border-t border-white/10 pt-4 sm:grid-cols-[1fr_auto] sm:items-end"><label className="text-xs font-medium text-white/60">Tempo <span className="ml-2 text-white">{tempo} BPM</span><Slider aria-label="Tempo" min={50} max={150} value={[tempo]} onValueChange={(value) => setTempo(Array.isArray(value) ? value[0] : tempo)} className="mt-3" /></label><div className="flex flex-wrap gap-2"><Button variant={countIn ? "default" : "outline"} aria-pressed={countIn} onClick={() => setCountIn((current) => !current)}><Timer className="size-4" />Count-in</Button><Button variant={metronome ? "default" : "outline"} aria-pressed={metronome} onClick={() => setMetronome((current) => !current)}><Drum className="size-4" />Metronome</Button><Button variant={looping ? "default" : "outline"} onClick={() => setLooping((current) => !current)}><SkipBack className="size-4" />Loop {looping ? "on" : "off"}</Button><Button variant="ghost" onClick={() => { setLoopStart(0); setLoopEnd(loopLength); }}>Reset loop</Button></div></div><div className="mt-4 flex flex-wrap items-center gap-2 border-t border-white/10 pt-4"><span className="mr-1 text-xs font-medium text-white/50">Jump to measure</span>{Array.from({ length: measureCount }, (_, index) => index + 1).map((measure) => <Button key={measure} variant={measure === currentMeasure ? "secondary" : "ghost"} size="sm" aria-label={`Jump to measure ${measure}`} onClick={() => jumpToMeasure(measure)}>{measure}</Button>)}</div><p className="mt-3 text-xs text-white/40" aria-live="polite">{countIn ? "Count-in on" : "Count-in off"} · {metronome ? "Metronome on" : "Metronome off"} · Space play/pause · R restart · Loop repeats {loopStart.toFixed(0)}–{loopEnd.toFixed(0)} seconds</p></div></section></main>;
}
