...
import { noteIndexAtBeat, noteIndicesCrossed, wrapBeat } from "@/lib/practice-playback";
...
export function PracticePage({ sheetId }: { sheetId: string }) {
  ...
  const duration = useMemo(() => Math.max(loopLength, (sheet?.noteEvents.at(-1)?.start ?? 0) + (sheet?.noteEvents.at(-1)?.duration ?? 1)), [sheet]);
  ...
  function tick(timestamp: number) {
    ...
    const frameBeats = (timestamp - (lastFrame.current ?? timestamp)) / 1000 * beatsPerSecond;
    ...
    if (countInRemaining.current > 0) {
      ...
      if (completed > 0) {
        ...
        countInRemaining.current = Math.max(0, countInRemaining.current - completed);
    ...
    const next = looping ? wrapBeat(nextElapsed, loopStart, loopEnd) : nextElapsed;
    ...
    const crossed = next < previousElapsed && looping
      ? [...noteIndicesCrossed(notes, previousElapsed, loopEnd), ...noteIndicesCrossed(notes, loopStart - 0.0001, next)]
